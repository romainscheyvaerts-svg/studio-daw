...
export class VocalDoublerNode {
  ...
  private applyParams() {
    ...
    if (isEnabled) {
      ...
      this.wetGainL.gain.setTargetAtTime(gainL, now, 0.05);
      this.wetGainR.gain.setTargetAtTime(gainR, now, 0.05);
      this.pannerL.pan.setTargetAtTime(-width, now, 0.1);
      this.pannerR.pan.setTargetAtTime(width, now, 0.1);
      this.modGainL.gain.setTargetAtTime(0.0005 + (detune * 0.0015), now, 0.1);
      this.modGainR.gain.setTargetAtTime(0.0005 + (detune * 0.0015), now, 0.1);
...
const DoublerKnob: React.FC<{ 
  ...
  const handleMouseDown = (e: React.MouseEvent) => {
    ...
    const onMouseMove = (m: MouseEvent) => {
      const deltaY = (startY - m.clientY) / 150;
